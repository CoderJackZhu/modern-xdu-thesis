// 本科入口完整编译与版式验收样例
#import "../bachelor.typ": documentclass

#let (doc, cover, abstract, abstract-en, outline-page, mainmatter,
  appendix, acknowledgement, references, 引用) = documentclass(
  info: (
    ...
    abstract: [
      ...
      的基本原理、实现方法和实验结果，并总结后续可以改进的方向。本段用于验证本科模板的摘要
      版式、首行缩进和一点五倍行距，实际使用时应替换为不少于三百字的真实摘要内容。
    ],
    abstract-en: [
      Video-based human action recognition is an important research topic in artificial
      intelligence. This sample verifies the undergraduate abstract layout, indentation,
      line spacing, running header, and Roman page numbering. Replace it with the complete
      English abstract when writing the thesis.
    ],
...
基础任务，在智能安防、人机交互和智能家居等领域具有应用价值。本段包含足够多的文字，用于
测量正文十二磅字号下相邻基线是否稳定保持二十三点四磅，并验证首行缩进两个汉字。
