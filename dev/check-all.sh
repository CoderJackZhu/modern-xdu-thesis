...
sec "0) 发布包载荷隐私扫描"
if python3 dev/check-release-payload.py && python3 dev/test-release-payload.py; then
  ok "发布包排除规则、隐私扫描与负向测试通过"
else
  bad "发布包隐私扫描失败"
fi
...
if bash dev/test-bachelor-cover-assets.sh >/dev/null 2>&1; then
  ok "本科封面用户素材与占位符回归通过"
else
  bad "本科封面用户素材与占位符回归失败"
fi
